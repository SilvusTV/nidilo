...
> **Périmètre du pilote : les données médicales sont désactivées.** Les champs d’allergie,
> traitements, température et consignes médicales ne doivent pas être utilisés, y compris dans
> les notes libres. Les règles médicales ci-dessous préparent uniquement une éventuelle phase
> ultérieure, après validation juridique et technique.
...
| Santé, allergies, traitements et consignes médicales                    |                                    Non collectées pendant le pilote |                                                                                        Sans objet | Sans objet                                             | Aucun                                                | Fonctionnalité désactivée                  |
...
Pour un enfant, la purge doit supprimer dans une transaction logique : profil, transmissions, messages, liens vers les responsables, médias S3 et métadonnées associées. Si les fonctions médicales sont activées ultérieurement, leurs données devront rejoindre cette purge. Les éléments contractuels ayant une durée distincte doivent être préalablement séparés du dossier courant.

## 6. Données de santé et mineurs — hors pilote
